...
void syntax_error();
void initialize();
// make a new node for the syntax tree
...
void checkForwardReference()
{
	...
	while (id)
	{
		// check forward referencea (for struct & enum types)
		if (id->level < current_level)
			break;
		t = id->type;
		if (id->kind == ID_NULL)
...
// get the type of struct id
A_TYPE *getTypeOfStructOrEnumRefIdentifier(T_KIND k, char *s, ID_KIND kk)
{
	A_TYPE *t;
	A_ID *id;
	id = searchIdentifier(s, current_id);
	// check if it is an exisiting identifier
	if (id)
		if (id->kind == kk && id->type->kind == k)
			return (id->type);
		else
			syntax_error(11, s);
	// make a new struct (or enum) identifier (with the type kind k), return its
	t = makeType(k);
	id = makeIdentifier(s);
	id->kind = kk;
	id->type = t;
...
A_ID *setDeclaratorListSpecifier(A_ID *id, A_SPECIFIER *p)
{
	...
	while (a)
	{
		...
		// set : the element type of the declarator
		// determine the kind of the identifier
		if (p->stor == S_TYPEDEF)
			a->kind = ID_TYPE;
...
A_TYPE *setTypeNameSpecifier(A_TYPE *t, A_SPECIFIER *p)
{
	...
	if (p->stor)
		syntax_error(20, p->stor);
	setDefaultSpecifier(p);
	t = setTypeElementType(t, p->type);
	return (t);
}
// set element type of the type t
...
A_TYPE *setTypeStructOrEnumIdentifier(T_KIND k, char *s, ID_KIND kk)
{
	...
	if (a)
		if (a->kind == kk && a->type->kind == k)
			...
			syntax_error(12, s);
	// make a new struct (or enum) identifier and return its type
	id = makeIdentifier(s);
	t = makeType(k);
	id->type = t;
	id->kind = kk;

...
void gen_literal_table()
{
        ...
        fprintf(fout," %fn", literal_table[i].value.f);
...
void gen_expression(A_NODE *node)
{
        ...
        case T_ENUM:
        case T_POINTER:
            gen_code_(LOD, id-> level, id-> address); 
            break;
        case T_ARRAY:
        if (id-> kind==ID_VAR)
        ...
        case T_UNION:
             gen_code_i(LDA, id-> level, id->address);
             i=id->type-> size;
             gen_code_i(LDI,0,1%4?i/4+1:i/4);
        break;
...
if (!isArrayType(node->type)) {
    ...
    if (i==1)
      ...
      gen_code_i(LDIB,0,0);
       ...
       gen_code_i(LDI,0, i%4?i/4+1:1/4);}
break;
...
case N_EXP_MUL :
        ...
        break;
case N_EXP_DIV :
        gen_expression(node-> llink); gen_expression(node-> rlink);
        if (isFloatType(node->type))
        gen_code_i(DIVF,0,0);
        else
        gen_code_i(DIVI,0,0);
...
void gen_statement(A_NODE *node, int cont_label, int break_label, A_SWITCH sw[],int *sn)
{
                ...
                *sn=*sn+1;
                ...
                case N_STMT_EXPRESSION:
                        ...
                        i=n->type-> size;
                        if (i)
                        gen_code_i(POP,0,1%4?i/4+1:i/4);
                        break;
case N_STMT_IF:
gen_expression (node-> llink);
...
gen_code_l(JMP,0,l2=get_label());
gen_statement(node-> rlink, cont_label,12, switch_table, &switch_no); 
gen_label_number (l1);
...
gen_code_(SWLABEL,0,switch_table[i].label);}
...
i=n->llink->type->size;
if (i)
gen_code_i(POP,0,1%4?1/4+1:1/4); }
gen_label_number(l1=get_label());
l2=get_label();
if (n->clink) {
